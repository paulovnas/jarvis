...
pub(super) fn move_project(
    ...
    tx.execute(
        "UPDATE navigation_selection SET workspace_id = ?1 WHERE project_id = ?2",
        params![workspace_id, id],
    )?;
...
pub(super) fn usage(
    ...
    home: &Path,
) -> Result<Vec<WorkspaceStorage>, LibraryError> {
    ...
    library
        ...
        .map(|workspace| {
            let projects = library
                ...
                .map(|project| {
                    let conversations: Vec<_> = library
                        .conversations
                        .iter()
            ...
            Ok(WorkspaceStorage {
                conversations: projects.iter().map(|p| p.conversations).sum(),
                bytes: projects.iter().map(|p| p.bytes).sum(),
                workspace,
                projects,
            })
        })
...
pub async fn move_project_workspace(
    ...
    let result = run(app.clone(), state.inner().clone(), move |db, _| {
        move_project(db, &id, &workspace_id)
    })
    .await?;
...
#[tauri::command]
pub async fn get_workspace_storage(
    app: AppHandle,
    state: State<'_, AppState>,
) -> Result<Vec<WorkspaceStorage>, LibraryError> {
