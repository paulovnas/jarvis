use super::*;
use crate::library::{deletion::DeleteTarget, workspaces::{move_project, usage}};
...
fn moving_a_project_keeps_session_identity_source_and_selected_conversation() {
    ...
    let chat = insert_conversation(&mut db, &home.0, &project.id, "Chat").unwrap().conversations[0].clone();
    let path = session_path(&home.0, &project.id, &chat.id, false).unwrap();
    let journal = fs::read(&path).unwrap();
    let workspace = insert_workspace(&mut db, "Trabalho").unwrap().selection.workspace_id.unwrap();
    select_item(&mut db, &home.0, LibraryTarget::Conversation(chat.id.clone())).unwrap();
    let moved = move_project(&mut db, &project.id, &workspace).unwrap();
    assert_eq!(moved.selection.workspace_id.as_deref(), Some(workspace.as_str()));
    assert_eq!(moved.selection.conversation_id.as_deref(), Some(chat.id.as_str()));
    assert_eq!(read_conversation(&db, &home.0, &chat.id).unwrap().project.path, project.path);
...
fn workspace_storage_counts_owned_history_and_attachments_not_project_source() {
    ...
    fs::write(Path::new(&project.path).join("source.txt"), vec![0; 100_000]).unwrap();
    #[cfg(unix)]
...
fn workspace_deletion_removes_all_histories_and_keeps_other_workspaces_and_source() {
    ...
    let mut db = database();
    let project = setup_project(&mut db, &home);
    let second = insert_project(&mut db, &project.workspace_id, &home.project("second")).unwrap().selection.project_id.unwrap();
    let first_chat = insert_conversation(&mut db, &home.0, &project.id, "A").unwrap().selection.conversation_id.unwrap();
    ...
    let other = insert_workspace(&mut db, "Other").unwrap().selection.workspace_id.unwrap();
    let third = insert_project(&mut db, &other, &home.project("third")).unwrap().selection.project_id.unwrap();
    let surviving = insert_conversation(&mut db, &home.0, &third, "C").unwrap().selection.conversation_id.unwrap();
    select_item(&mut db, &home.0, LibraryTarget::Conversation(first_chat.clone())).unwrap();
    let target = DeleteTarget::Workspace(project.workspace_id);
    assert_eq!(crate::library::deletion::conversation_ids(&db, &target).unwrap().len(), 2);
    ...
    assert_eq!(deleted.selection.workspace_id.as_deref(), Some(other.as_str()));
    assert!(deleted.selection.conversation_id.is_none());
    assert_eq!(fs::read_to_string(source).unwrap(), "preserve");
    assert!(!home.0.join(".jarvis/sessions").join(&project.id).join(format!("{first_chat}.jsonl")).exists());
    assert!(read_conversation(&db, &home.0, &surviving).is_ok());
    assert_eq!(crate::library::deletion::delete(&mut db, &home.0, &target).unwrap(), deleted);
}
...
fn workspace_deletion_rolls_back_all_projects_if_a_later_journal_cannot_be_staged() {
    ...
    let a = insert_conversation(&mut db, &home.0, &first.id, "A").unwrap().selection.conversation_id.unwrap();
    let second = insert_project(&mut db, &first.workspace_id, &home.project("second")).unwrap().selection.project_id.unwrap();
    let b = insert_conversation(&mut db, &home.0, &second, "B").unwrap().selection.conversation_id.unwrap();
    ...
    #[cfg(unix)] {
        ...
        assert!(crate::library::deletion::delete(&mut db, &home.0, &DeleteTarget::Workspace(first.workspace_id)).is_err());
...
fn workspace_deletion_restores_every_staged_journal_after_metadata_failure() {
    ...
    let mut db = database();
    let project = setup_project(&mut db, &home);
    let a = insert_conversation(&mut db, &home.0, &project.id, "A").unwrap().selection.conversation_id.unwrap();
    let second = insert_project(&mut db, &project.workspace_id, &home.project("second")).unwrap().selection.project_id.unwrap();
    let b = insert_conversation(&mut db, &home.0, &second, "B").unwrap().selection.conversation_id.unwrap();
    ...
    assert!(crate::library::deletion::delete(&mut db, &home.0, &DeleteTarget::Workspace(project.workspace_id)).is_err());
