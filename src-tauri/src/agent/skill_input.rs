...
pub(super) fn normalize(
    ...
    if parts.len() > 256 || ids(&parts).len() > 8 {
        return Err(AgentError::new("invalid_message", "Selecione até 8 skills por mensagem."));
    }
    if parts.is_empty() { return Ok((content, parts)); }
...
fn ids(parts: &[MessagePart]) -> Vec<String> {
    parts.iter().filter_map(|part| match part { MessagePart::Skill { id, .. } => Some(id.clone()), _ => None }).collect()
}
...
mod tests {
    ...
    async fn explicit_skills_are_user_input_keep_badges_and_survive_queue_restart() {
        ...
        let available = crate::skills::active(&fixture.root, &fixture.root).await.unwrap();
        ...
        assert!(turn.wire[0]["content"].as_str().unwrap().contains("Use the fixture workflow."));
        assert!(turn.wire[0]["content"].as_str().unwrap().contains(dir.to_str().unwrap()));
        ...
        assert!(normalize(&fixture.root, &fixture.root, content, turn.turn.parts.clone()).is_err());
    }
    ...
    fn legacy_queue_and_turn_input_remain_readable_and_mentions_are_bounded() {
        ...
        assert_eq!(normalize(home, home, "Hello".into(), vec![]).unwrap().0, "Hello");
        assert!(normalize(home, home, "Hello".into(), vec![MessagePart::Skill { id: "unknown".into(), name: "test".into() }; 9]).is_err());
    }
}
