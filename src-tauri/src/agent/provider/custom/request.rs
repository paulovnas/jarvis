...
fn cache_breakpoints(config: &Config, model: &Model, body: &mut Value) -> Result<(), AgentError> {
    ...
    let known_host = endpoint.scheme() == "https"
        ...
        && matches!(
            endpoint.host_str(),
            Some("api.anthropic.com" | "openrouter.ai")
        );
    ...
    if !known_host
        || !(model.id.starts_with("claude-") || model.id.starts_with("anthropic/claude-"))
    {
        return Ok(());
    }
    ...
    match config.protocol {
        Protocol::AnthropicMessages => {
            ...
            if let Some(message) = body["messages"]
                .as_array_mut()
                .and_then(|messages| messages.last_mut())
            {
                ...
                if let Some(block) = message["content"]
                    .as_array_mut()
        ...
        Protocol::OpenaiCompletions if endpoint.host_str() == Some("openrouter.ai") => {
            if let Some(messages) = body["messages"].as_array_mut() {
                let last = messages
                    .iter()
                    .rposition(|m| matches!(m["role"].as_str(), Some("user" | "assistant")));
                for (index, message) in messages.iter_mut().enumerate() {
                    if index != 0 && Some(index) != last {
                        continue;
                    }
                    if message["content"].is_string() {
                        message["content"] = json!([{"type":"text","text":message["content"]}]);
                    ...
                    if let Some(block) = message["content"]
                        .as_array_mut()
                        .and_then(|blocks| blocks.iter_mut().rev().find(|b| b["type"] == "text"))
                    {
