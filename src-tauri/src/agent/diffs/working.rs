...
fn failure() -> AgentError {
    AgentError::new(
        "diff_unavailable",
        "Não foi possível conferir as alterações pendentes da sessão.",
    )
}
...
async fn git(root: &Path, args: &[&str]) -> Result<(bool, String), AgentError> {
    let mut child = crate::background::tokio_command("git")
...
impl Repository {
    pub async fn open(root: &Path) -> Result<Option<Self>, AgentError> {
        ...
        // Git prints the toplevel with forward slashes and no Windows verbatim
        // prefix, while callers pass a canonicalized \\?\ root. Canonicalize so
        // strip_prefix below compares identical forms on every platform.
        let root = std::fs::canonicalize(path.trim_end_matches('\n')).map_err(|_| failure())?;
        Ok(Some(Self {
            root,
    ...
    pub async fn contents(&self, root: &Path, path: &str) -> Result<Option<String>, AgentError> {
        ...
        let relative = absolute.strip_prefix(&self.root).map_err(|_| failure())?;
        // Git pathspecs and the {head}:{path} object use forward slashes. Join
        // the path components so Windows separators normalize, while a literal
        // backslash inside a Unix filename is preserved (it is a valid character
        // there, not a separator).
        let relative: String = relative
            .components()
            .map(|component| component.as_os_str().to_str().ok_or_else(failure))
...
async fn current(root: &Path, path: &str) -> Result<Option<String>, AgentError> {
    ...
    let file = tokio::fs::File::open(absolute)
        .await
        .map_err(|_| failure())?;
    if !file.metadata().await.map_err(|_| failure())?.is_file() {
        return Err(failure());
    }
    let mut bytes = vec![];
...
fn project_lines(before: &str, after: &str, owned: &HashSet<usize>) -> HashSet<usize> {
    diff(before, after)
        .iter_all_changes()
        .filter_map(|change| {
            (change.tag() == ChangeTag::Equal && owned.contains(&change.old_index()?))
                .then_some(change.new_index()?)
        })
        .collect()
}

pub(super) fn pending(file: &FileRevision, head: Option<&str>, live: Option<&str>) -> FileRevision {
    let empty_created = file.before.is_none()
        && file.after.as_deref() == Some("")
        && head.is_none()
        && live == Some("");
    let empty_deleted = file.before.as_deref() == Some("")
        && file.after.is_none()
        && head == Some("")
        && live.is_none();
    ...
    let surviving: HashSet<_> = diff(after, live)
        .iter_all_changes()
        .filter(|change| change.tag() == ChangeTag::Equal)
        .filter_map(|change| change.old_index())
        .collect();
    ...
    for op in session_diff.ops() {
        ...
        let survives =
            op.new_range().is_empty() || op.new_range().any(|index| surviving.contains(&index));
    ...
    for change in diff(head, live).iter_all_changes() {
        let owned = match change.tag() {
            ChangeTag::Insert => change
                .new_index()
                .is_some_and(|index| added.contains(&index)),
        ...
        if change.tag() == ChangeTag::Equal
            || (change.tag() == ChangeTag::Insert && !owned)
            || (change.tag() == ChangeTag::Delete && owned)
        {
            baseline.push_str(change.value());
        }
    }
    let mut result = FileRevision::new(
        file.path.clone(),
        Some(baseline),
        Some(live.to_owned()),
        "conversation",
    ...
    result.before.hash(&mut hash);
    result.after.hash(&mut hash);
...
pub(super) async fn files(
    session: &Session,
    only: Option<&str>,
) -> Result<Vec<FileRevision>, AgentError> {
    let revisions: Vec<_> = session
        .data
        .lock()
        ...
        .files
        .values()
        .filter(|file| file.base != "unknown" && only.is_none_or(|path| path == file.path))
        .cloned()
        .collect();
