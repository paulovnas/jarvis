...
fn commit(root: &Path) {
    run(root, &["-c", "core.hooksPath=/dev/null", "commit", "-qm", "test"]);
}
fn revision(before: &str, after: &str) -> FileRevision {
    FileRevision::new("a.txt".into(), Some(before.into()), Some(after.into()), "conversation")
}
...
fn intersects_session_lines_with_partial_commits_and_other_dirty_changes() {
    ...
    assert!(rows.iter().any(|row| row.kind == "added" && row.text == "TWO"));
    assert!(rows.iter().any(|row| row.kind == "removed" && row.text == "two"));
    assert!(!rows.iter().any(|row| row.kind != "context" && (row.text == "ONE" || row.text.contains("external") || row.text.contains("other change"))));
    assert!(!pending(&file, Some(live), Some(live)).changed());
    assert!(!pending(&file, Some("external dirty\nONE\nTWO\n"), Some("external dirty\nONE\nTWO\nunrelated\n")).changed());
    assert!(!pending(&revision("original\n", "session\n"), Some("original\n"), Some("overwritten externally\n")).changed());
}
...
async fn live_git_excludes_other_files_staged_commits_and_keeps_post_commit_edits() {
    ...
    run(&fixture.root, &["add", "a.txt"]); commit(&fixture.root);
    std::fs::write(fixture.root.join("a.txt"), "ONE\ntwo\n").unwrap();
    record(&session, revision("one\ntwo\n", "ONE\ntwo\n")).await.unwrap();
    std::fs::write(fixture.root.join("unrelated.txt"), "dirty").unwrap();
    run(&fixture.root, &["add", "a.txt"]);
    assert_eq!(files(&session, None).await.unwrap().len(), 1, "staging is not committing");
    commit(&fixture.root);
    ...
    assert_eq!((pending[0].additions, pending[0].deletions), (Some(1), Some(1)));
...
async fn supports_unborn_repositories_deleted_files_and_literal_paths() {
    ...
    let session = crate::agent::tests::session(&fixture);
    init(&fixture.root);
    let path = "strange [name]\n.txt";
    std::fs::write(fixture.root.join(path), "new\n").unwrap();
    record(&session, FileRevision::new(path.into(), None, Some("new\n".into()), "conversation")).await.unwrap();
    assert_eq!(files(&session, None).await.unwrap()[0].additions, Some(1));
    run(&fixture.root, &["add", "--", path]); commit(&fixture.root);
    assert!(files(&session, None).await.unwrap().is_empty());
    std::fs::remove_file(fixture.root.join(path)).unwrap();
    record(&session, FileRevision::new(path.into(), Some("new\n".into()), None, "conversation")).await.unwrap();
...
async fn legacy_unknown_edits_never_claim_live_disk_changes_as_session_work() {
    ...
    session.data.lock().unwrap().extras.files.insert("a.txt".into(), FileRevision::new("a.txt".into(), None, None, "unknown"));
    assert!(files(&session, None).await.unwrap().is_empty());
    std::fs::write(fixture.root.join("a.txt"), "session\n").unwrap();
    record(&session, revision("external\n", "session\n")).await.unwrap();
    let changes = files(&session, None).await.unwrap();
    assert_eq!((changes[0].additions, changes[0].deletions), (Some(1), Some(1)));
}
