...
fn png() -> Vec<u8> {
    ...
    image::DynamicImage::new_rgb8(8, 6).write_to(&mut bytes, image::ImageFormat::Png).unwrap();
    bytes.into_inner()
}
fn event() -> Value {
...
fn account(state: &AppState, home: &Path, alias: &str, kind: &str) {
    state.with_connection(home, |db| db.execute("INSERT INTO provider_accounts(alias,provider_kind,account_id) VALUES(?1,?2,?1)", [alias,kind]).map_err(|_| AgentError::storage())).unwrap();
}
...
fn settings_default_to_off_and_only_enable_active_antigravity() {
    ...
    assert_eq!(save(&state, home.path(), Some("google".into())).unwrap().model.as_deref(), Some(MODEL));
    assert!(enabled(&state, home.path()));
    state.with_connection(home.path(), |db| db.execute("UPDATE provider_accounts SET enabled=0 WHERE alias='google'", []).map_err(|_| AgentError::storage())).unwrap();
    assert!(!enabled(&state, home.path()));
    assert!(save(&state, home.path(), Some("google".into())).is_err());
    state.with_connection(home.path(), |db| db.execute("DELETE FROM provider_accounts WHERE alias='google'", []).map_err(|_| AgentError::storage())).unwrap();
...
async fn disabled_generation_fails_without_credentials_or_network() {
    ...
    let result = execute(&AppState::default(), &OpenAiCodexState::default(), home.path(), &"a".repeat(32), &json!({"prompt":"Uma árvore"}), signal).await;
...
fn request_uses_fixed_model_and_owned_references_only() {
    ...
    let args = arguments(&json!({"prompt":"Recolorir","aspect_ratio":"16:9","image_ids":[item.id]})).unwrap();
    let body = request_body(&credential, home.path(), &conversation, &args).unwrap();
    assert_eq!(body["model"], MODEL);
    assert_eq!(body["project"], "project");
    assert_eq!(body["request"]["generationConfig"]["responseModalities"], json!(["IMAGE"]));
...
fn output_persists_images_without_binary_payloads_or_private_thoughts() {
    ...
    assert_eq!(attachments::metadata(home.path(), &conversation, id).unwrap().kind, "image");
    assert_eq!(attachments::bounded_read(&attachments::location(home.path(), &conversation, id).unwrap().join("source"), attachments::MAX_BYTES).unwrap(), png());
}

#[test]
fn malformed_blocked_empty_and_incomplete_results_fail() {
    assert!(Output::default().event(&json!({"response":{"promptFeedback":{"blockReason":"SAFETY"}}})).is_err());
    assert!(Output::default().event(&json!({"error":{"message":"sensitive detail"}})).unwrap_err().message.find("sensitive").is_none());
    let mut empty = Output::default();
    empty.event(&json!({"response":{"candidates":[{"finishReason":"STOP"}]}})).unwrap();
    assert!(empty.validate().is_err());
    let mut broken = event();
    broken["response"]["candidates"][0]["content"]["parts"][0]["inlineData"]["data"] = json!("broken");
    assert!(Output::default().event(&broken).is_err());
    let mut incomplete = event();
    incomplete["response"]["candidates"][0].as_object_mut().unwrap().remove("finishReason");
    let mut output = Output::default(); output.event(&incomplete).unwrap();
...
async fn fixture_response(body: String, status: u16) -> reqwest::Response {
    ...
    std::thread::spawn(move || {
        ...
        let mut request = [0;1024]; let _ = socket.read(&mut request);
        let header = format!("HTTP/1.1 {status} OK\r\nContent-Type: text/event-stream\r\nContent-Length: {}\r\nConnection: close\r\n\r\n",body.len());
        socket.write_all(header.as_bytes()).unwrap();
        for chunk in body.as_bytes().chunks(19) { if socket.write_all(chunk).is_err() { break; } }
    });
...
async fn streaming_images_handle_fragmented_sse_eof_and_cancellation() {
    ...
    let (_send, signal) = watch::channel(false);
    let output = receive(fixture_response(body.clone(), 200).await, signal).await.unwrap();
    assert_eq!(output.images, vec![png()]);
    let (_send, signal) = watch::channel(true);
    assert_eq!(receive(fixture_response(body, 200).await, signal).await.err().unwrap().code, "cancelled");
    let (_send, signal) = watch::channel(false);
    let error = receive(fixture_response("sensitive server response".into(), 429).await, signal).await.err().unwrap();
