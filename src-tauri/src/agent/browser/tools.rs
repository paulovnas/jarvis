...
pub(in crate::agent) fn definitions(mode: Mode) -> Vec<Value> {
    ...
    let specs = [
        ...
        ("screenshot", "Capture the native viewport as a conversation image attachment. Use the vision tool with the returned attachment.id to actually inspect the screenshot. Do not infer pixels from the filename or page text.", json!({"id":id}), vec!["id"]),
