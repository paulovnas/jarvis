...
pub(super) async fn request(
    ...
    if exec.id == "main" {
        return Err(invalid(
            "O agente principal usa ask_user para esclarecer decisões.",
        ));
    }
    ...
    exec.hub.mutate(|state| {
        if let Some(job) = state.jobs.get_mut(&exec.id) {
            if job.status == Status::Waiting {
                job.status = Status::Running;
            }
        }
        if result.is_err() {
            state.guidance.remove(&id);
        }
        Ok(())
    })?;
    result
...
pub(super) fn respond(exec: &Execution, id: &str, answer: &str) -> Result<String, AgentError> {
    exec.hub.mutate(|state| {
        let request = state
            .guidance
            .get_mut(id)
            .ok_or_else(|| invalid("Pedido de orientação não encontrado."))?;
        if request.to != exec.id
            || request.run_id != state.run_id
            || request.answer.is_some()
            || !state
                .jobs
                .get(&request.from)
                .is_some_and(|job| job.status.active() && job.run_id == state.run_id)
        {
            return Err(invalid(
                "Responda apenas ao pedido atual de um agente filho ativo.",
            ));
        }
...
mod tests {
    ...
    async fn guidance_is_durable_correlated_parent_only_and_resumes_the_designer() {
        ...
        assert!(respond(
            &execution(&hub, "unrelated", Role::Planner),
            id,
            "Wrong answer"
        )
        .is_err());
