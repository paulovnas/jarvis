...
pub(super) fn validate(denied: &[String]) -> Result<(), AgentError> {
    if denied.len() > 256
        || denied.iter().collect::<BTreeSet<_>>().len() != denied.len()
        || denied.iter().any(|name| {
            required(name)
                || name.is_empty()
                || name.len() > 128
                || !(name == "mcp_*"
    ...
    {
        return Err(invalid(
            "Permissões inválidas. Os recursos obrigatórios do Core não podem ser desativados.",
        ));
    }
    Ok(())
}
...
fn description(name: &str) -> &'static str {
    match name {
        ...
        "edit" => "Editar trechos de arquivos.",
        "apply_patch" => "Aplicar uma alteração transacional em vários arquivos.",
        "lsp_definition" => "Encontrar a definição de um símbolo de código.",
        ...
        "jarvis_propose_agent" => {
            "Propor a criação ou edição supervisionada de um agente customizado."
        }
        "jarvis_propose_flow" => {
        ...
        "ctx_index" => "Indexar conteúdo sem carregar tudo no contexto.",
        "ctx_stats" => "Consultar a redução de conteúdo do Context-mode.",
        "ctx_execute" => "Processar dados e comandos fora do contexto.",
        "ctx_execute_file" => "Processar um arquivo e retornar somente o resultado.",
        "ctx_batch_execute" => "Executar e indexar consultas em lote.",
        "ctx_fetch_and_index" => "Buscar e indexar uma página web.",
        "process_start" => "Iniciar um serviço persistente no projeto.",
        "process_list" => "Listar serviços da conversa.",
        "process_output" => "Consultar a saída de um serviço.",
        "process_check_port" => "Verificar se uma porta está disponível.",
        ...
        "browser_press" => "Enviar uma tecla para a página.",
        "browser_scroll" => "Rolar a página.",
        "beads_list" => "Listar tarefas do projeto.",
        "beads_ready" => "Encontrar tarefas disponíveis.",
        "beads_show" => "Consultar os detalhes de uma tarefa.",
        "beads_create" => "Criar tarefas e épicos.",
        "beads_update" => "Atualizar tarefas e notas.",
        ...
        "browser_console" => "Consultar erros e mensagens do console.",
        "browser_close" => "Fechar uma aba do navegador.",
        "hub_complete" => "Entregar o resultado da etapa ao fluxo.",
        "workflow_check" => "Executar as verificações suportadas do projeto.",
        "hub_list" => "Consultar agentes e seus estados.",
        "hub_wait" => "Aguardar o resultado de subagentes.",
        "hub_send" => "Enviar uma orientação a outro agente.",
        "hub_spawn" => "Iniciar um subagente nos fluxos nativos.",
        "hub_retry" => "Retomar o trabalho de um subagente.",
        "hub_cancel" => "Cancelar um subagente e seus descendentes.",
        "hub_request_guidance" => "Solicitar uma decisão ao coordenador.",
        "hub_respond_guidance" => "Responder à dúvida de um subagente.",
        "validation_publish" => "Publicar itens para a validação do usuário.",
        "design_brief" => "Registrar as decisões de design da etapa.",
...
fn permission(name: &str, group: &str) -> Permission {
    Permission {
        ...
        name: name.into(),
        group: group.into(),
        description: description(name).into(),
        ...
        .into_iter()
        .filter(|c| custom::capability_allows(*c, name))
        .collect(),
    }
}

pub(crate) fn builtin_permissions() -> Vec<Permission> {
    ...
    let groups = [
        ...
        (
            ...
            [
                processes::definitions(Mode::Build),
                terminals::definitions(Mode::Build),
            ]
            .concat(),
        ),
    ...
    for (group, tools) in groups {
        for tool in tools {
            if let Some(name) = tool["name"].as_str() {
                result.insert(name.to_owned(), permission(name, group));
            }
        }
    }
    for name in crate::core::context::TOOLS {
        result.insert(name.into(), permission(name, "Context-mode · Core"));
    }
...
pub async fn get_agent_tool_permissions(
    ...
    mcp: tauri::State<'_, crate::mcp::McpState>,
) -> Result<Vec<Permission>, AgentError> {
    use tauri::Manager;
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        {
            if let Some(check) = &server.last_check {
                for name in &check.tools {
                    let mut tool = permission(
                        &crate::mcp::runtime::wire_name(&server, name),
                        &format!("MCP · {}", server.name),
                    );
                    tool.name.clone_from(name);
                    tool.description = if server.enabled {
                        "Ferramenta descoberta neste MCP."
                    } else {
                        "O MCP está desativado nas configurações."
                    }
                    .into();
                    result.push(tool);
                }
...
mod tests {
    ...
    fn every_catalog_tool_has_enforced_capabilities_and_mandatory_core_is_not_configurable() {
        ...
        assert!(required("update_tasks"));
        assert!(validate(&["update_tasks".into()]).is_err());
