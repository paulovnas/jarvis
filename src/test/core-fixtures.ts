...
export function coreFixture(installed = true): CoreSnapshot {
  ...
  ].map(item => ({ ...item, id: item.id as CoreSnapshot["items"][number]["id"], installed, installedVersion: installed ? "1.0.0" : null, latestVersion: "1.0.0", updateAvailable: false, stage: null, error: null })) };
}
