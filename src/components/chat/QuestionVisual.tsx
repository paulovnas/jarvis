...
export function QuestionVisual({ preview, label }: { preview: QuestionPreview; label: string }) {
  ...
  if (preview.type === "palette") return <span className="block overflow-hidden rounded-md border border-border bg-background"><span className="flex h-16">{preview.colors.map((color, index) => <span key={index} className="flex-1" style={{ backgroundColor: color }} title={color} />)}</span><span className="block px-3 py-3 text-sm font-medium text-foreground">{preview.sample}</span><span className="block px-3 pb-2 font-mono text-[10px] text-muted-foreground">{preview.colors.join(" · ")}</span></span>;
