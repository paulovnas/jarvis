import { AlertCircle, Check, ChevronRight, FilePenLine, FileText, FolderSearch, Search, Terminal, Wrench } from "lucide-react";
  ...
  return (
    <Collapsible data-testid={`tool-call-${tool.id}`} className="min-w-0">
      ...
      <CollapsibleContent className="flex min-w-0 flex-col gap-3 py-2 pl-6 text-xs">
        ...
        {tool.output && <div><p className="mb-1">Resultado</p><pre className="max-h-64 overflow-auto rounded-md bg-muted p-3 text-foreground">{tool.output}</pre></div>}
