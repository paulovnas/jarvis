    ...
    <DropdownMenuContent align="start" side="top" sideOffset={10} className="instrument-panel w-[380px] max-w-[calc(100vw-32px)] bg-card p-2">
      <DropdownMenuGroup className="grid grid-cols-2 gap-2" aria-label="Fluxos">
        {FLOWS.map(flow => <DropdownMenuItem key={flow.value} aria-label={flow.title} onClick={() => onChange(flow.value)} className="group relative flex cursor-pointer flex-col items-start gap-2.5 rounded-md border p-3 whitespace-normal transition-colors motion-reduce:transition-none" style={{ borderColor: flow.value === value ? `${flow.color}80` : "var(--border)", backgroundColor: flow.value === value ? `${flow.color}12` : undefined }}>
          ...
          <span className="space-y-1"><span className="block text-xs font-medium" style={{ color: flow.color }}>{flow.title}</span><span className="block text-[11px] leading-4 text-muted-foreground">{flow.description}</span></span>
        </DropdownMenuItem>)}
      </DropdownMenuGroup>
    </DropdownMenuContent>
  </DropdownMenu>;
