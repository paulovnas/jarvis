...
const STEPS = ["Boas-vindas", "Core", "Provedores", "Seu espaço"];
const ignoreOpen = () => {};
...
export function Onboarding({ saving, onComplete }: { saving: boolean; onComplete: (workspaceName: string) => Promise<void> }) {
  ...
  const blocked = saving || (step === 1 && (!core.snapshot?.ready || core.busy)) || (step === 2 && (!providerReady || providerBusy)) || (step === 3 && (!core.snapshot?.ready || !providerReady));
  return <main className="flex min-h-0 flex-1 items-center justify-center p-4 sm:p-6">
    <Card className="instrument-panel flex max-h-full w-full max-w-5xl flex-col gap-0 overflow-hidden py-0">
      <CardHeader className="shrink-0 gap-5 border-b border-border px-6 py-5">
        ...
        <ol aria-label="Etapas da configuração" className="grid grid-cols-4 gap-2">{STEPS.map((label, index) => <li key={label} aria-current={step === index ? "step" : undefined} className={`flex items-center gap-2 border-t-2 pt-3 text-xs ${step === index ? "border-primary font-medium text-primary" : step > index ? "border-onedark-green/60 text-onedark-green" : "border-border text-muted-foreground"}`}><span className="font-mono text-[10px]">{index < step ? <Check className="size-3" /> : `0${index + 1}`}</span>{label}</li>)}</ol>
      </CardHeader>
      <CardContent key={step} className="min-h-0 overflow-y-auto overscroll-contain px-6 py-6 motion-safe:animate-in motion-safe:fade-in motion-safe:duration-200">
        ...
        {step === 1 && <div className="space-y-5"><div className="space-y-2"><h1 className="text-2xl font-medium tracking-tight">Prepare suas ferramentas</h1><p className="text-sm text-muted-foreground">Cinco componentes locais em ~/.jarvis.</p></div><CorePanel core={core} setup /></div>}
        {step === 2 && <div className="space-y-5"><div className="space-y-2"><h1 className="text-2xl font-medium tracking-tight">Conecte sua inteligência</h1><p className="text-sm text-muted-foreground">Adicione um provedor e escolha como usar Web Search e Vision.</p></div><Suspense fallback={<CardsSkeleton label="Carregando provedores" columns />}><Providers open embeddedProviders onOpenChange={ignoreOpen} onAccountsChange={setAccounts} onBusyChange={setProviderBusy} /></Suspense></div>}
        {step === 3 && <div className="mx-auto max-w-lg space-y-6 py-4"><div className="flex size-12 items-center justify-center rounded-lg border border-onedark-green/25 bg-onedark-green/10 text-onedark-green"><Check className="size-6" /></div><div className="space-y-2"><h1 className="text-2xl font-medium tracking-tight">Tudo pronto para começar</h1><p className="text-sm leading-6 text-muted-foreground">Dê um nome ao seu workspace. Depois, adicione a pasta do seu primeiro projeto.</p></div><div className="space-y-2"><Label htmlFor="first-workspace">Workspace padrão</Label><Input id="first-workspace" placeholder="Pessoal" value={workspace} maxLength={120} disabled={saving} onChange={event => setWorkspace(event.target.value)} onKeyDown={event => { if (event.key === "Enter" && !blocked) void onComplete(workspace); }} /></div></div>}
      </CardContent>
      <CardFooter className="shrink-0 justify-between border-t border-border bg-sidebar/40 px-6 py-4"><div>{step > 0 && <Button variant="ghost" disabled={saving || core.busy} onClick={() => setStep(step - 1)}><ArrowLeft className="size-4" />Voltar</Button>}</div><Button disabled={blocked} aria-busy={saving} onClick={() => { if (step < 3) setStep(step + 1); else void onComplete(workspace); }}>{step === 3 ? <><FolderPlus className="size-4" />{saving ? "Preparando…" : "Começar"}</> : <>Avançar<ArrowRight className="size-4" /></>}</Button></CardFooter>
    </Card>
  </main>;
}
