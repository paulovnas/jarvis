...
function FileButton({ file, selected, onClick }: { file: FileChange; selected?: boolean; onClick: () => void }) {
  ...
  return <Button variant="ghost" onClick={onClick} aria-pressed={selected} aria-label={`Alterações em ${file.path}`} className={`h-auto w-full cursor-pointer justify-start gap-2 px-2 py-2 text-xs ${selected ? "bg-primary/10 text-primary" : ""}`}>
    ...
    <Hint content={file.path} whenTruncated><span className="min-w-0 flex-1 text-left"><span className="block truncate">{name}</span>{parts.length > 0 && <span className="block truncate text-[10px] text-muted-foreground">{parts.join("/")}</span>}</span></Hint>
    <Counts {...file} />
  </Button>;
}

function DiffView({ conversationId, file, revision, projectPath }: { conversationId: string; file: FileChange; revision: number; projectPath?: string }) {
  const [result, setResult] = useState<{ diff?: FileDiff; error?: string } | null>(null);
  const [attempt, setAttempt] = useState(0);
  const filePath = projectPath ? resolveProjectFilePath(projectPath, file.path) : null;
  useEffect(() => {
    let active = true;
...
export function ChangedFiles({ files, conversationId, projectPath }: { files: FileChange[]; conversationId: string; projectPath?: string }) {
  ...
  return <>
    ...
    <Dialog open={selected !== null} onOpenChange={open => { if (!open) setSelected(null); }}>
      <DialogContent showCloseButton={false} className="flex h-[min(82vh,820px)] w-[calc(100vw-3rem)] max-w-none flex-col gap-0 overflow-hidden p-0 sm:max-w-6xl">
        <DialogHeader className="shrink-0 border-b border-border px-5 py-4 pr-14"><div className="flex min-w-0 items-center justify-between gap-3"><DialogTitle>Arquivos alterados</DialogTitle>{projectPath && <Button variant="outline" size="sm" className="shrink-0 cursor-pointer gap-2 text-xs" aria-label="Abrir pasta do projeto" onClick={() => { void invoke("open_conversation_path", { conversationId, path: null }).catch(error => toast.error(libraryError(error, "Não foi possível abrir a pasta do projeto."))); }}><FolderOpen aria-hidden="true" className="size-3.5" />Abrir pasta</Button>}</div><DialogDescription className="text-xs">{files.length} {files.length === 1 ? "arquivo alterado" : "arquivos alterados"} nesta conversa. Selecione um arquivo para revisar o diff.</DialogDescription></DialogHeader>
        <Hint content="Fechar alterações"><DialogClose render={<Button variant="ghost" size="icon" className="absolute right-3 top-3 cursor-pointer" aria-label="Fechar alterações" />}><X className="size-4" /></DialogClose></Hint>
        <div className="flex min-h-0 flex-1 flex-col sm:flex-row">
          ...
          {file && selected !== null && <DiffView key={`${conversationId}/${file.path}/${file.revision ?? 0}`} conversationId={conversationId} file={file} revision={file.revision ?? 0} projectPath={projectPath} />}
        </div>
      </DialogContent>
    </Dialog>
  </>;
