  ...
  return <>
    ...
    <Dialog open={selected !== null} onOpenChange={open => { if (!open) setSelected(null); }}>
      <DialogContent showCloseButton={false} className="flex h-[min(82vh,820px)] w-[calc(100vw-3rem)] max-w-none flex-col gap-0 overflow-hidden p-0 sm:max-w-6xl">
        <DialogHeader className="shrink-0 border-b border-border px-5 py-4 pr-14"><div className="flex min-w-0 items-center justify-between gap-3"><DialogTitle>Arquivos alterados</DialogTitle>{projectPath && <Button variant="outline" size="sm" className="shrink-0 cursor-pointer gap-2 text-xs" aria-label="Abrir pasta do projeto" onClick={() => { void openPath(projectPath).catch(() => toast.error("Não foi possível abrir a pasta do projeto.")); }}><FolderOpen aria-hidden="true" className="size-3.5" />Abrir pasta</Button>}</div><DialogDescription className="text-xs">{files.length} {files.length === 1 ? "arquivo alterado" : "arquivos alterados"} nesta conversa. Selecione um arquivo para revisar o diff.</DialogDescription></DialogHeader>
