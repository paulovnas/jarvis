...
describe("Changed file review", () => {
  beforeEach(() => { call.mockReset().mockImplementation(async (command, args) => command === "open_conversation_path" ? undefined : diff((args as { path: string }).path)); });
  ...
  it("opens the project folder and selected file with the system defaults", async () => {
    ...
    expect(call).toHaveBeenCalledWith("open_conversation_path", { conversationId: "c1", path: null });
    await user.click(within(dialog).getByRole("button", { name: "Abrir src/file-2.ts no editor padrão" }));
    expect(call).toHaveBeenCalledWith("open_conversation_path", { conversationId: "c1", path: "src/file-2.ts" });
  });
});
