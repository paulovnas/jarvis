      ...
      {state.repositories.map(repository => {
        ...
        return <Card key={repository.id} role="article" aria-label={`GitHub ${repository.name}`} size="sm" className="gap-0 overflow-hidden rounded-md py-0">
          ...
          {repository.available ? <>
            ...
            {repository.remoteUrl && <Hint content={repository.remoteUrl} whenTruncated><p className="mt-1.5 cursor-default truncate font-mono text-[9px] text-muted-foreground">{repository.remoteUrl}</p></Hint>}
