...
export function ContextUsage({ context, live, onCompact, compacting = false, disabled = false, external = false }: {
  context: ReturnType<typeof conversationContext>; live?: ContextInfo;
  onCompact?: () => Promise<boolean>; compacting?: boolean; disabled?: boolean; external?: boolean;
}) {
  ...
  return <footer aria-label="Contexto da conversa" aria-busy={busy} data-compacting={busy || undefined} className="context-meter shrink-0 border-t border-border bg-sidebar px-4 py-3" style={{ "--context-color": color } as CSSProperties}>
    ...
    <div className="mt-2 flex items-center gap-2">
      {!external && <Hint content="Compactar contexto"><Button variant="ghost" size="icon" aria-label="Compactar contexto" disabled={disabled || busy || !onCompact} className="size-6 shrink-0 cursor-pointer text-muted-foreground" onClick={() => setConfirming(true)}><RefreshCw aria-hidden="true" className={`size-3.5 ${busy ? "animate-spin motion-reduce:animate-none" : ""}`} /></Button></Hint>}
    ...
    {external && <p className="mt-1 text-right text-[10px] text-muted-foreground">Contexto gerenciado pelo Claude Code</p>}
