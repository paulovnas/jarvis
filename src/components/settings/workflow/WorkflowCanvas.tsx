...
import { Flag, GripVertical, Maximize, Minus, Plus } from "lucide-react";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import type { CustomAgent, CustomFlow } from "@/core/workflow-catalog";
...
type AgentNode = Node<{ label: string; description: string; entry: boolean; appearance?: WorkflowAppearance | null }, "agent">;
const AgentBlock = memo(function AgentBlock({ data, selected }: NodeProps<AgentNode>) {
  return <Card className={`workflow-node w-56 cursor-pointer gap-0 py-0 ${selected ? "border-primary ring-1 ring-primary/30" : "border-border"}`}>
    ...
    <CardContent className="p-3"><div className="flex items-center gap-2"><WorkflowIdentityIcon appearance={data.appearance} className="size-4 shrink-0" /><span className="min-w-0 flex-1 truncate text-xs font-medium">{data.label}</span><GripVertical className="size-3.5 text-muted-foreground" /></div>
      {data.entry && <Badge variant="outline" className="mt-2 gap-1 border-primary/40 text-[10px] text-primary"><Flag className="size-2.5" />Início</Badge>}
      ...
      <div className="mt-3 flex justify-between border-t border-border pt-2 font-mono text-[9px]"><span className="text-primary">Concluído →</span><span className="text-destructive">Correção ↓</span></div>
    </CardContent>
    <Handle type="source" position={Position.Right} id="next" className="cursor-pointer !bg-primary" aria-label="Saída de conclusão" />
    <Handle type="source" position={Position.Bottom} id="onRework" className="cursor-pointer !bg-destructive" aria-label="Saída de correção" />
  </Card>;
});
const nodeTypes = { agent: AgentBlock };
function reconcileNodes(previous: AgentNode[], flow: CustomFlow, agents: CustomAgent[], selected: string | null): AgentNode[] {
  return flow.steps.map(step => {
    const old = previous.find(node => node.id === step.id);
    const agent = agents.find(item => item.id === step.agentId);
    const data = { label: agent?.name ?? "Agente indisponível", description: step.instructions || agent?.description || "", entry: step.id === flow.entry, appearance: agent?.appearance };
    const sameData = old && old.data.label === data.label && old.data.description === data.description && old.data.entry === data.entry && old.data.appearance === data.appearance;
...
export default function WorkflowCanvas({ flow, agents, selected, onSelect, onChange, disabled }: { flow: CustomFlow; agents: CustomAgent[]; selected: string | null; onSelect: (id: string | null) => void; onChange: (flow: CustomFlow) => void; disabled: boolean }) {
  ...
  const onNodesChange = useCallback<OnNodesChange<AgentNode>>(changes => {
    ...
    if (positions.some(change => change.position)) onChange({ ...flow, steps: flow.steps.map(step => { const change = positions.find(item => item.id === step.id); return change?.position ? { ...step, position: change.position } : step; }) });
  }, [disabled, flow, onChange, onSelect]);
  const edges = useMemo<Edge[]>(() => flow.steps.flatMap(step => (["next", "onRework"] as const).flatMap(kind => step[kind] ? [{ id: `${step.id}/${kind}`, source: step.id, sourceHandle: kind, target: step[kind], label: kind === "next" ? "Concluído" : "Correção", type: "smoothstep", style: { stroke: kind === "next" ? "var(--primary)" : "var(--destructive)" }, labelStyle: { fill: "var(--muted-foreground)", fontSize: 10 }, labelBgStyle: { fill: "var(--card)" } }] : [])), [flow.steps]);
  return <div className="workflow-canvas h-full min-h-64 overflow-hidden rounded-md border border-border bg-sidebar" aria-label="Canvas do fluxo"><ReactFlowProvider><ReactFlow<AgentNode>
    ...
    onConnect={connection => {
      ...
      onChange({ ...flow, steps: flow.steps.map(step => step.id === connection.source ? { ...step, [connection.sourceHandle === "next" ? "next" : "onRework"]: connection.target } : step) });
    }}
