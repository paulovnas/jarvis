...
export function CustomAgentEditor({ initial, accounts, saving, onSave, onClose, creating }: { initial: CustomAgent; accounts: ProviderAccount[]; saving: boolean; onSave: (agent: CustomAgent) => Promise<boolean>; onClose: () => void; creating: boolean }) {
  ...
  return <><Dialog open onOpenChange={open => { if (!open) close(); }}>
    <DialogContent className="dark flex max-h-[92dvh] w-[calc(100vw-2rem)] flex-col gap-0 overflow-hidden p-0 sm:max-w-[1000px]">
      ...
      <div className="space-y-2 rounded-md border border-border bg-sidebar p-3"><p className="text-xs font-medium">Provedor e modelo</p><div className="flex flex-wrap items-center gap-2"><ExecutorModelPicker modelGroups={accountGroups(accounts)} selection={executionSelection(agent.model)} disabled={saving} ariaLabel="Modelo do agente customizado" onSelect={next => patch({ model: executionChoice(next) })} /><Button type="button" variant="outline" size="sm" className="cursor-pointer text-xs" onClick={() => patch({ model: null })}>Usar modelo do chat</Button></div><p className="text-xs text-muted-foreground">{agent.model ? `Modelo fixo: ${executionLabel(agent.model)}` : "Este agente usa o provedor e o modelo selecionados no composer."}</p></div>
