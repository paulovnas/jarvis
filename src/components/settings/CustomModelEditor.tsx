  ...
  return <Collapsible open={open} onOpenChange={onOpenChange} className="min-w-0"><Card size="sm" className="gap-0">
    ...
    <CardContent className="flex flex-col gap-3 pt-3">
      ...
      {loading ? <div role="status" aria-label={`Buscando dados do modelo ${number}`} className="flex gap-2"><Skeleton className="h-5 w-28" /><Skeleton className="h-5 w-24" /><Skeleton className="h-5 w-20" /></div> : model.contextWindow > 0 && <div className="flex flex-wrap items-center gap-1.5 text-[10px]">
        <Badge variant="outline" className="font-mono text-[10px]">{model.contextWindow.toLocaleString("pt-BR")} contexto</Badge><Badge variant="outline" className="font-mono text-[10px]">{model.maxOutputTokens.toLocaleString("pt-BR")} saída</Badge>{model.supportsTools && <Badge variant="outline" className="text-[10px]">Ferramentas</Badge>}{model.supportsImages && <Badge variant="outline" className="text-[10px]">Imagens</Badge>}{model.reasoning !== "none" && model.reasoningLevels.length > 0 && <Hint content={model.reasoningLevels.join(", ")}><Badge variant="outline">Raciocínio · {model.reasoningLevels.join(" / ")}</Badge></Hint>}
