  ...
  return <Collapsible open={open} onOpenChange={onOpenChange} className="min-w-0"><Card size="sm" className="gap-0">
    <CardHeader className="grid-cols-[minmax(0,1fr)_auto] items-center gap-2">
      ...
      <CardAction className="row-span-1 self-center">
        <Hint content={`Remover modelo ${number}`}><Button type="button" variant="ghost" size="icon-sm" aria-label={`Remover modelo ${number}`} disabled={!canRemove || saving} onClick={onRemove}><Trash2 data-icon="inline-start" className="text-destructive" /></Button></Hint>
      </CardAction>
    </CardHeader>
    ...
    <CardContent className="flex flex-col gap-3 pt-3">
      <div className="flex items-end gap-2"><div className="min-w-0 flex-1"><Field label={`ID do modelo ${number}`} help="Copie o ID da página do modelo no OpenRouter (por exemplo, fabricante/modelo). Ao sair deste campo, o Jarvis consulta o catálogo público; não envia mensagens nem usa sua chave." value={model.id} disabled={saving} placeholder="fabricante/modelo" onChange={e => { request.current += 1; onPendingChange(false); setLookup(null); edit({ id: e.target.value }); }} onBlur={() => void lookupModel()} /></div>{canLookup && <Hint content="Preencher pelo ID"><Button type="button" variant="outline" size="icon" aria-label={`Preencher modelo ${number} pelo ID`} disabled={saving || loading || !model.id.trim()} onClick={() => void lookupModel(true)}><Sparkles className="size-3.5 text-primary" /></Button></Hint>}</div>
      {loading ? <div role="status" aria-label={`Buscando dados do modelo ${number}`} className="flex gap-2"><Skeleton className="h-5 w-28" /><Skeleton className="h-5 w-24" /><Skeleton className="h-5 w-20" /></div> : model.contextWindow > 0 && <div className="flex flex-wrap items-center gap-1.5 text-[10px]">
        <Badge variant="outline" className="font-mono text-[10px]">{model.contextWindow.toLocaleString("pt-BR")} contexto</Badge><Badge variant="outline" className="font-mono text-[10px]">{model.maxOutputTokens.toLocaleString("pt-BR")} saída</Badge>{model.supportsTools && <Badge variant="outline" className="text-[10px]">Ferramentas</Badge>}{model.supportsImages && <Badge variant="outline" className="text-[10px]">Imagens</Badge>}{model.reasoning !== "none" && model.reasoningLevels.length > 0 && <Badge variant="outline">Raciocínio · {model.reasoningLevels.join(" / ")}</Badge>}
