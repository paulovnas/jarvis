...
export function WorkspaceSettings() {
  ...
  return <div className="space-y-4">
    <div className="flex items-center justify-between gap-3"><div><h2 className="micro-label text-muted-foreground">Histórico armazenado</h2><p className="mt-1 text-xs text-muted-foreground">Mensagens, anexos e memória das conversas. As pastas dos projetos não entram no cálculo.</p></div><Hint content="Atualizar armazenamento"><Button variant="ghost" size="icon" aria-label="Atualizar armazenamento" disabled={loading || pending} onClick={() => { setLoading(true); setError(null); void load(); }}><RefreshCw className="size-4" /></Button></Hint></div>
    ...
    {loading ? <CardsSkeleton label="Medindo históricos dos workspaces" columns /> : <div className="grid gap-4 md:grid-cols-2">{items.map(item => <Card key={item.workspace.id} className="min-w-0 gap-4 bg-card">
      <CardHeader className="flex flex-row items-center gap-3"><Layers className="size-5 shrink-0 text-onedark-cyan" /><Hint content={item.workspace.name} whenTruncated><CardTitle className="min-w-0 flex-1 truncate text-sm">{item.workspace.name}</CardTitle></Hint><Hint content={`Excluir ${item.workspace.name}`}><Button variant="ghost" size="icon" aria-label={`Excluir workspace ${item.workspace.name}`} className="shrink-0 text-muted-foreground hover:text-destructive" onClick={() => { setError(null); setDeletion(item); }}><Trash2 className="size-4" /></Button></Hint></CardHeader>
