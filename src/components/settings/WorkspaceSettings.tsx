...
export function WorkspaceSettings() {
  ...
  return <div className="space-y-4">
    ...
    {loading ? <CardsSkeleton label="Medindo históricos dos workspaces" columns /> : <div className="grid gap-4 md:grid-cols-2">{items.map(item => <Card key={item.workspace.id} className="min-w-0 gap-4 bg-card">
      ...
      <CardContent className="space-y-4"><div className="flex flex-wrap gap-4 font-mono text-xs text-muted-foreground"><span className="flex items-center gap-1.5"><Folder className="size-3.5" />{item.projects.length} projetos</span><span className="flex items-center gap-1.5"><MessageSquare className="size-3.5" />{item.conversations} conversas</span><span className="flex items-center gap-1.5 text-onedark-cyan" title="Espaço ocupado pelos históricos"><HardDrive className="size-3.5" />{storageSize(item.bytes)} em históricos</span></div>
        <div className="divide-y divide-border" aria-label={`Históricos dos projetos de ${item.workspace.name}`}>{item.projects.map(p => <div key={p.project.id} className="flex min-w-0 items-center gap-3 py-2.5 text-xs"><p className="min-w-0 flex-1 truncate font-medium" title={p.project.name}>{p.project.name}</p><span className="shrink-0 font-mono text-[10px] text-muted-foreground">{p.conversations} conversas</span><span className="shrink-0 font-mono tabular-nums" title={`Histórico armazenado por ${p.project.name}`}>{storageSize(p.bytes)}</span></div>)}</div>
      </CardContent>
    </Card>)}</div>}
