...
export function WorkspaceSettings() {
  ...
  return <div className="space-y-4">
    <div className="flex items-center justify-between gap-3"><div><h2 className="micro-label text-muted-foreground">Históricos por workspace</h2><p className="mt-1 text-xs text-muted-foreground">Conversas, anexos e memória local. Não inclui os arquivos dos projetos.</p></div><Button variant="ghost" size="icon" aria-label="Atualizar armazenamento" disabled={loading || pending} onClick={() => { setLoading(true); setError(null); void load(); }}><RefreshCw className="size-4" /></Button></div>
    ...
    {loading ? <CardsSkeleton label="Medindo históricos dos workspaces" columns /> : <div className="grid gap-4 md:grid-cols-2">{items.map(item => <Card key={item.workspace.id} className="min-w-0 gap-4 bg-card">
      <CardHeader className="flex flex-row items-center gap-3"><Layers className="size-5 shrink-0 text-onedark-cyan" /><CardTitle className="min-w-0 flex-1 truncate text-sm" title={item.workspace.name}>{item.workspace.name}</CardTitle><Button variant="ghost" size="icon" aria-label={`Excluir workspace ${item.workspace.name}`} className="shrink-0 text-muted-foreground hover:text-destructive" onClick={() => { setError(null); setDeletion(item); }}><Trash2 className="size-4" /></Button></CardHeader>
      <CardContent className="space-y-4"><div className="flex flex-wrap gap-4 font-mono text-xs text-muted-foreground"><span className="flex items-center gap-1.5"><Folder className="size-3.5" />{item.projects.length} projetos</span><span className="flex items-center gap-1.5"><MessageSquare className="size-3.5" />{item.conversations} chats</span><span className="flex items-center gap-1.5 text-onedark-cyan"><HardDrive className="size-3.5" />{storageSize(item.bytes)}</span></div>
        <div className="divide-y divide-border">{item.projects.map(p => <div key={p.project.id} className="flex min-w-0 items-center gap-3 py-2.5 text-xs"><div className="min-w-0 flex-1"><p className="truncate font-medium" title={p.project.name}>{p.project.name}</p><p className="mt-1 truncate font-mono text-[10px] text-muted-foreground" title={p.project.path}>{p.project.path}</p></div><span className="shrink-0 font-mono text-[10px] text-muted-foreground">{p.conversations} chats</span><span className="shrink-0 font-mono tabular-nums">{storageSize(p.bytes)}</span></div>)}</div>
      </CardContent>
    </Card>)}</div>}
