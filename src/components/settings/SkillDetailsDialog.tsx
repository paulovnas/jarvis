...
export function SkillDetailsDialog({ selection, onClose }: { selection: SkillSelection | null; onClose: () => void }) {
  const [result, setResult] = useState<{ selection: SkillSelection; detail?: SkillDetail; error?: string } | null>(null);
  useEffect(() => {
    if (!selection) return;
    let active = true;
    ...
    void request.then(value => { if (active) setResult({ selection, detail: skillDetailSchema.parse(value) }); }).catch(cause => { if (active) setResult({ selection, error: skillError(cause) }); });
    return () => { active = false; };
  }, [selection]);
  const detail = result?.selection === selection ? result?.detail : undefined;
  const error = result?.selection === selection ? result?.error : undefined;
  return <Dialog open={selection !== null} onOpenChange={open => { if (!open) onClose(); }}>
    <DialogContent className="dark flex max-h-[88vh] flex-col gap-4 sm:max-w-3xl">
      ...
      {error ? <p role="alert" className="text-sm text-destructive">{error}</p> : !detail ? <DocumentSkeleton label="Carregando skill" /> : <>
        <div className="flex min-w-0 flex-wrap items-center gap-2">
          ...
          {detail.path && <span className="w-full truncate font-mono text-[11px] text-muted-foreground" title={detail.path}>{detail.path}</span>}
        </div>
