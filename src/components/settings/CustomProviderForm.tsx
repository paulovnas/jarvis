...
export function CustomProviderForm({ account, onSaved, onCancel, onBusyChange }: { account?: ProviderAccount; onSaved: (account: ProviderAccount) => void; onCancel: () => void; onBusyChange?: (busy: boolean) => void }) {
  ...
  return <TooltipProvider delay={150}><form onSubmit={event => void save(event)} className="flex min-w-0 flex-col gap-4" aria-busy={saving}>
    <fieldset disabled={saving} className="flex min-w-0 flex-col gap-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2"><h3 className="micro-label text-muted-foreground">Modelos</h3><div className="flex items-center gap-1">{supportsModelLookup(config.baseUrl) && <Hint content="Abrir catálogo OpenRouter"><Button type="button" variant="ghost" size="icon-sm" aria-label="Abrir catálogo OpenRouter" onClick={() => { void openUrl("https://openrouter.ai/models").catch(() => toast.error("Não foi possível abrir o catálogo.")); }}><ExternalLink data-icon="inline-start" /></Button></Hint>}<Button type="button" variant="outline" size="sm" disabled={saving || config.models.length >= 100} onClick={() => { const key = nextKey.current++; setModelKeys(keys => [...keys, key]); setConfig(c => ({ ...c, models: [...c.models, emptyModel()] })); setExpandedModel(key); }}><Plus data-icon="inline-start" />Adicionar modelo</Button></div></div>
