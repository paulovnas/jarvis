...
export function SkillsSettings({ onCountChange }: { onCountChange?: (count: number) => void }) {
  ...
  const [localSnapshot, setLocalSnapshot] = useState<SkillSnapshot | null>(() => bootstrap?.resources.skills ?? null);
  const snapshot = bootstrap ? bootstrap.resources.skills : localSnapshot;
  ...
  const loadInitially = useRef(!(bootstrap?.resources.loaded.skills ?? false));
  const checkInitially = useRef(!(bootstrap?.resources.checked.skills ?? false));
  const initialSnapshot = useRef(snapshot);
  const update = useCallback((value: unknown, checked?: boolean) => {
    const parsed = skillsSnapshotSchema.parse(value);
    if (mounted.current) {
      currentSnapshot.current = parsed;
      if (updateBootstrapSkills) updateBootstrapSkills(parsed, checked);
      else setLocalSnapshot(parsed);
      onCountChange?.(parsed.skills.length);
      setError(null);
    }
    return parsed;
  }, [onCountChange, updateBootstrapSkills]);
  const checkUpdates = useCallback(async () => {
    if (checkPending.current || mutationPending.current) return;
    ...
    try {
      ...
      update({
        ...checked,
        includeAgents: current?.includeAgents ?? checked.includeAgents,
        skills: checked.skills.map(skill => enabled.has(skill.id)
          ? { ...skill, enabled: enabled.get(skill.id) ?? skill.enabled }
          : skill),
      }, true);
    } catch (cause) {
      if (mounted.current) toast.error(skillError(cause));
    } finally {
      checkPending.current = false;
      if (mounted.current) setCheckingUpdates(false);
    }
  }, [update]);
  ...
  }, [checkUpdates, onCountChange, update]);
  async function perform(key: string, action: () => Promise<void>) {
    if (mutationPending.current) return;
    mutationPending.current = true; setBusy(key);
    try { await action(); } catch (cause) { if (mounted.current) toast.error(skillError(cause)); }
    finally { mutationPending.current = false; if (mounted.current) setBusy(null); }
  }
  async function upgrade(ids: string[]) {
    if (checkingUpdates) return;
    await perform("update", async () => {
      const result = skillUpdateSchema.parse(await invoke("update_skills", { ids })); update(result.snapshot, true);
  ...
  return <div className="flex flex-col gap-4">
    ...
    {checkingUpdates && <Alert role="status" aria-live="polite" className="border-onedark-yellow/25 bg-onedark-yellow/5 text-onedark-yellow">
      ...
      <AlertDescription className="text-xs">Você pode continuar ativando ou desativando skills. Atualizações e exclusões ficam disponíveis ao terminar.</AlertDescription>
    </Alert>}
    ...
    <AlertDialog open={deleting !== null} onOpenChange={open => { if (!open && !mutationPending.current) setDeleting(null); }}>
      <AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Excluir {deleting?.name}?</AlertDialogTitle><AlertDialogDescription>{deleting?.linked ? "O vínculo será removido. A pasta de destino será preservada." : deleting?.origin === "jarvis" ? "A pasta da skill e seus arquivos serão excluídos definitivamente." : "A pasta compartilhada será excluída definitivamente, afetando também outros agentes que a utilizam."}</AlertDialogDescription></AlertDialogHeader>
        ...
        <AlertDialogFooter><Button variant="ghost" className="cursor-pointer" disabled={!!busy} onClick={() => setDeleting(null)}>Cancelar</Button><Button data-confirm-action variant="destructive" className="cursor-pointer" disabled={!!busy || checkingUpdates} onClick={() => { if (deleting && !checkingUpdates) void perform("delete", async () => { update(await invoke("delete_skill", { id: deleting.id })); setDeleting(null); toast.success("Skill excluída"); }); }}>{busy === "delete" && <Spinner />}Excluir skill</Button></AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
  </div>;
}
