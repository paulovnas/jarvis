...
export function SkillsSettings({ onCountChange }: { onCountChange?: (count: number) => void }) {
  ...
  return <div className="flex flex-col gap-4">
    ...
    {error ? <p role="alert" className="text-xs text-destructive">{error}</p> : !snapshot ? <CardsSkeleton label="Carregando skills" columns /> : visible.length === 0 ? <p className="py-8 text-center text-sm text-muted-foreground">{query ? "Nenhuma skill encontrada." : "Nenhuma skill instalada."}</p> : <div className="grid grid-cols-1 items-start gap-3 sm:grid-cols-2">{visible.map(skill => <Card key={skill.id} className="min-w-0 gap-0 overflow-hidden p-0">
