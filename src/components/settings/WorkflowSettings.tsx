...
export function WorkflowSettings({ accounts, initialTab = "flows" }: { accounts: ProviderAccount[]; initialTab?: "flows" | "agents" }) {
  ...
  const customSection = (kind: "agent" | "flow") => <section aria-label={kind === "flow" ? "Fluxos customizados" : "Agentes customizados"} className="space-y-3">
    ...
    {catalog.error ? <div role="alert" className="space-y-2 text-xs text-destructive"><p>{catalog.error}</p><Button variant="outline" size="sm" className="cursor-pointer" onClick={() => void catalog.refresh()}>Tentar novamente</Button></div> : !catalog.data ? <div role="status" aria-label="Carregando customizados" className="space-y-2"><Skeleton className="h-20" /><Skeleton className="h-20" /></div> : <>
      {(kind === "flow" ? catalog.data.flows : catalog.data.agents).map(item => <Card key={item.id} className="instrument-panel flex-row items-center gap-1 p-2">
        ...
        </Button><Hint content={`Duplicar ${item.name}`}><Button variant="ghost" size="icon" className="size-7 cursor-pointer text-muted-foreground" aria-label={`Duplicar ${item.name}`} onClick={() => { const copy = { ...item, id: customId(), name: `${item.name.slice(0, 85)} (cópia)` }; const revision = catalog.data?.revision ?? 0; setEditor(kind === "flow" ? { kind, value: copy as CustomFlow, revision, creating: true } : { kind, value: copy as CustomAgent, revision, creating: true }); }}><Copy className="size-3.5" /></Button></Hint><Hint content={`Excluir ${item.name}`}><Button variant="ghost" size="icon" className="size-7 cursor-pointer text-muted-foreground hover:text-destructive" aria-label={`Excluir ${item.name}`} onClick={() => setDeleting({ kind, id: item.id, name: item.name, revision: catalog.data?.revision ?? 0 })}><Trash2 className="size-3.5" /></Button></Hint>
      </Card>)}
  ...
  return <>
    <Tabs value={tab} onValueChange={setTab} className="gap-5"><TabsList aria-label="Workflow" aria-orientation="horizontal" className="h-9 flex-row! items-center bg-sidebar"><TabsTrigger value="flows" className="w-auto! flex-none! cursor-pointer justify-center! gap-2 px-4 text-xs"><Route />Fluxos</TabsTrigger><TabsTrigger value="agents" className="w-auto! flex-none! cursor-pointer justify-center! gap-2 px-4 text-xs"><Bot />Agentes</TabsTrigger></TabsList>
      ...
      <TabsContent value="agents" className="space-y-6"><section aria-label="Agentes Jarvis" className="space-y-3"><h3 className="micro-label text-muted-foreground">Jarvis</h3><p className="text-xs text-muted-foreground">Agentes do Jarvis · clique para consultar as instruções.</p>{roles.map(role => { const Icon = AGENT_ICONS[role]; const flow = role === "github" ? "publication" : "complete"; return <AgentInstructionsDialog key={role} flow={flow} role={role}><DialogTrigger render={<Button variant="ghost" />} className={`h-auto whitespace-normal ${mutedCard}`} aria-label={`Ver agente ${ROLE_LABELS[role]}`}><Icon className="size-5 shrink-0 opacity-65" /><span className="min-w-0 flex-1"><span className="block text-xs font-medium">{ROLE_LABELS[role]}</span><span className="mt-1 block text-[11px] leading-4">{AGENT_DESCRIPTIONS[role]}</span></span>{role === "github" && <Badge variant="outline" className="shrink-0 text-[9px] text-muted-foreground">Misto</Badge>}<LockKeyhole className="size-3.5 shrink-0" /></DialogTrigger></AgentInstructionsDialog>; })}</section>{customSection("agent")}</TabsContent>
    </Tabs>
