...
import { CAPABILITY_LABELS, customId, type BuiltinFlow, type CustomAgent, type CustomFlow } from "@/core/workflow-catalog";
...
export function WorkflowSettings({ accounts }: { accounts: ProviderAccount[] }) {
  ...
  const createAgent = () => { if (catalog.data) setEditor({ kind: "agent", creating: true, revision: catalog.data.revision, value: { id: customId(), name: "", description: "", instructions: "", capability: "read_only", model: null } }); };
    ...
    {catalog.error ? <div role="alert" className="space-y-2 text-xs text-destructive"><p>{catalog.error}</p><Button variant="outline" size="sm" className="cursor-pointer" onClick={() => void catalog.refresh()}>Tentar novamente</Button></div> : !catalog.data ? <div role="status" aria-label="Carregando customizados" className="space-y-2"><Skeleton className="h-20" /><Skeleton className="h-20" /></div> : <>
      {(kind === "flow" ? catalog.data.flows : catalog.data.agents).map(item => <Card key={item.id} className="instrument-panel flex-row items-center gap-1 p-2">
        <Button variant="ghost" className="h-auto min-w-0 flex-1 cursor-pointer justify-start gap-3 px-2 py-3 text-left whitespace-normal" onClick={() => { const revision = catalog.data?.revision ?? 0; setEditor(kind === "flow" ? { kind, value: item as CustomFlow, revision, creating: false } : { kind, value: item as CustomAgent, revision, creating: false }); }} aria-label={`Editar ${item.name}`}>
          <WorkflowIdentityIcon appearance={item.appearance} fallback={kind === "flow" ? flowAppearance : agentAppearance} className="size-5 shrink-0" /><span className="min-w-0 flex-1"><span className="block text-xs font-medium">{item.name}</span><span className="mt-1 block text-[11px] leading-4 text-muted-foreground">{item.description || (kind === "flow" ? "Fluxo personalizado" : "Agente personalizado")}</span><span className="mt-2 block font-mono text-[10px] text-muted-foreground">{"steps" in item ? `${item.steps.length} etapas` : CAPABILITY_LABELS[item.capability]}</span></span>
  ...
  return <>
    <Tabs value={tab} onValueChange={setTab} className="gap-5"><TabsList aria-label="Workflow" className="bg-sidebar"><TabsTrigger value="flows" className="cursor-pointer gap-2 px-4 text-xs"><Route />Fluxos</TabsTrigger><TabsTrigger value="agents" className="cursor-pointer gap-2 px-4 text-xs"><Bot />Agentes</TabsTrigger></TabsList>
    ...
    {editor?.kind === "flow" && <CustomFlowEditor initial={editor.value} creating={editor.creating} agents={catalog.data?.agents ?? []} saving={catalog.saving} onClose={() => setEditor(null)} onSave={async flow => { const saved = await catalog.mutate({ kind: "save_flow", flow }, editor.revision); if (saved) toast.success("Fluxo salvo."); return saved; }} />}
