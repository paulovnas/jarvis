      ...
      <CardContent className="space-y-3 pt-5">
        ...
        {repositories.repositories.length > 0 && <div className="grid gap-3 md:grid-cols-2">
          {repositories.repositories.map(repository => <article key={repository.id} aria-label={`Repositório ${repository.name}`} className="rounded-lg border border-border bg-sidebar/45 p-4 shadow-[inset_0_1px_0_rgb(255_255_255/0.035)]">
            <div className="flex items-start gap-3"><span className="flex size-8 shrink-0 items-center justify-center rounded-md bg-secondary text-onedark-cyan"><FolderGit2 className="size-4" /></span><div className="min-w-0 flex-1"><h3 className="truncate text-sm font-medium">{repository.name}</h3><p className="mt-0.5 truncate font-mono text-[10px] text-muted-foreground">{repository.path}</p></div><Button type="button" variant="ghost" size="icon-sm" className="cursor-pointer text-muted-foreground" aria-label={`Editar ${repository.name}`} onClick={() => setEditor({ id: repository.id, directory: repository.directory, name: repository.name, description: repository.description })}><Pencil className="size-3.5" /></Button><Button type="button" variant="ghost" size="icon-sm" className="cursor-pointer text-destructive" aria-label={`Remover ${repository.name}`} onClick={() => setRemoving(repository)}><Trash2 className="size-3.5" /></Button></div>
