...
export function ProjectDashboard({ project, onSelectSession, navigation, initialTab = "general" }: { project: Project; onSelectSession: (id: string) => void; navigation?: ReactNode; initialTab?: "general" | "beads" | "options" }) {
  ...
  return <main aria-label={`Detalhes de ${project.name}`} className="project-dashboard @container flex h-full min-h-0 flex-col overflow-hidden">
    <header className="flex shrink-0 items-center gap-3 border-b border-border px-6 py-5">
      ...
      <div className="min-w-0 flex-1"><p className="micro-label mb-1 text-muted-foreground">Projeto / Detalhes</p><h1 className="truncate text-xl font-semibold tracking-tight">{project.name}</h1><Hint content="Abrir no Finder/Explorador"><Button variant="ghost" size="sm" className="mt-0.5 h-auto max-w-full cursor-pointer justify-start gap-1.5 px-0 py-0.5 font-mono text-[11px] text-muted-foreground hover:bg-transparent hover:text-primary" aria-label={`Abrir pasta do projeto ${project.name}`} onClick={() => { void invoke("open_project_directory", { projectId: project.id }).catch(error => toast.error(libraryError(error, "Não foi possível abrir a pasta do projeto."))); }}><FolderOpen aria-hidden="true" className="size-3 shrink-0" /><span className="truncate">{project.path}</span></Button></Hint></div>
