  ...
  return <div className="mx-auto max-w-[1600px] space-y-5 p-5 @3xl:p-6">
    ...
    <div className="grid gap-5 @3xl:grid-cols-2">
      <Card className="dashboard-card gap-3"><CardHeader className="flex flex-row items-center justify-between"><CardTitle className="flex items-center gap-2 text-sm"><Cpu className="size-4 text-primary" />Modelos utilizados</CardTitle><span className="micro-label text-muted-foreground">Interações</span></CardHeader><CardContent className="max-h-64 space-y-4 overflow-y-auto">
        {models.length ? models.map(([model, count]) => <div key={model} className="space-y-2"><div className="flex items-start gap-3"><div className="min-w-0 flex-1"><Hint content={model} whenTruncated><p className="truncate font-mono text-xs">{model.slice(model.indexOf("/") + 1)}</p></Hint><p className="mt-1 truncate text-[10px] text-muted-foreground">{model.slice(0, model.indexOf("/"))}</p></div><span className="font-mono text-xs text-muted-foreground">{number(count)}</span></div><Progress value={count / Math.max(1, m.turns) * 100} className="h-1" aria-label={`${model}: ${count} interações`} /></div>) : <QuietEmpty text="Nenhum modelo utilizado" />}
      </CardContent></Card>
