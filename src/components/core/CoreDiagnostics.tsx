...
export default function CoreDiagnostics({ core, onClose }: { core: CoreController; onClose: () => void }) {
  ...
  const hasPendingError = snapshot?.items.some(item => item.error) === true;
  return <>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            {snapshot?.items.map(item => {
              ...
              return <Card key={item.id} className="instrument-panel gap-0 py-0"><CardContent className="p-4">
                ...
                {(!ready || item.error) && <div className="mt-4 flex flex-wrap gap-2 border-t border-border pt-3">
                  {!ready && <Button size="sm" variant="outline" disabled={busy} aria-label={`Reparar ${item.name}`} onClick={() => void repair(item.id, false)}><Wrench className="size-3" />Reparar</Button>}
